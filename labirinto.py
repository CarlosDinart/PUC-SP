...
class Labirinto:
    ...
    def eh_celula_ocupada(self, prox_passo, agente_id):
        ...
        i, j = nova_posicao.coord_turtle()  # linha e coluna da célula sob investigação
        for id in self.agentes:  # para cada k no dicionário (agentes)
            v, b = self.agentes[id].coord_turtle()
            if (v, b) == (i, j):
                return True
            else:
                return False
